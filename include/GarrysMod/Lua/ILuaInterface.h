#ifndef GARRYSMOD_LUA_ILUAINTERFACE_H
#define GARRYSMOD_LUA_ILUAINTERFACE_H
...
namespace GarrysMod
{
	namespace Lua
	{
		...
		public:
			...
			bool			AssertArgument( int stackPosition, int expectedType );

			ILuaObject*		GetGlobal( const char* name );

			void			SetGlobal( const char* name, LuaFunction f );
			...
			ILuaVector*		GetAllTableMembers( int iTable );
			void			DeleteLuaVector( ILuaVector* pVector );
			...
			const char*		GetTypeNameEx( int iType );
